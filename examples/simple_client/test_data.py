...
def main(args: Args) -> None:
    ...
    # MODIFIED: Initialize a rich console for clean printing
    console = rich.console.Console()
    console.rule("[bold cyan]Starting Policy Inference[/bold cyan]")
    ...
    for i in tqdm.trange(args.num_steps, desc="Running policy"):
        ...
        # --- MODIFIED: Real-time feedback of server actions ---
        actions = response.get('actions')
        # Use tqdm.write to avoid breaking the progress bar
        tqdm.tqdm.write(f"--- Step {i+1}/{args.num_steps} ---")
        if actions is not None:
            # Format actions for better readability
            if isinstance(actions, np.ndarray):
                # For numpy arrays, format them nicely
                formatted_actions = np.array2string(actions, precision=4, separator=', ', suppress_small=True)
                tqdm.tqdm.write(f"Received Actions: \n{formatted_actions}")
            else:
                # For lists or other types
                tqdm.tqdm.write(f"Received Actions: {actions}")
        else:
            tqdm.tqdm.write("[yellow]No 'actions' key in server response.[/yellow]")
        # ---------------------------------------------------------
