...
try:
    ...
    print("Warning: install the RealMan Robotic_Arm package to enable real robot control.")
...
def map_value(
    value: float, from_min: float, from_max: float, to_min: float, to_max: float, inverted: bool = False
) -> float:
...
def movej_mujoco(data: mujoco.MjData, target_angles_deg: list[float], arm_actuator_indices: range):
    ...
    if len(target_angles_deg) != len(arm_actuator_indices):
        logging.warning(
            f"[Sim] Mismatch between target angles ({len(target_angles_deg)}) and arm actuators ({len(arm_actuator_indices)})."
        )
        return
...
def get_mujoco_observation(model: mujoco.MjModel, data: mujoco.MjData, renderer: mujoco.Renderer) -> dict | None:
    # ... (function content unchanged)
    try:
        ...
        rotation = Rotation.from_quat(
            [flange_quat[1], flange_quat[2], flange_quat[3], flange_quat[0]]
        ...
        return {
            ...
            "prompt": "pick up the purple block",
        }
...
def main(args: Args) -> None:
    ...
    if args.run_robot:
        if not args.robot_ip:
            logger.error("--robot-ip is required when --run-robot is set.")
            return
    ...
    try:
        with mujoco.viewer.launch_passive(model, data) as viewer:
            ...
            while viewer.is_running():
                ...
                if (
                    not isinstance(action_sequence, (list, np.ndarray))
                    or len(action_sequence) == 0
                    or not all(len(sub) == 7 for sub in action_sequence)
                ):
